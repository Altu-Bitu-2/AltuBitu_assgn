...
bool cmp(const pair<int, ci>& c1, const pair<int, ci>& c2) {
    if (c1.second.first != c2.second.first) { //first끼리 비교해서 다르면
        return c1.second.first < c2.second.first; //first끼리 비교
    }
    //fisrt끼리 비교해서 같으면
    return c1.second.second < c2.second.second; //second끼리 비교
}
...
int main() {
    ...
    for (auto iter = candidate.begin(); iter != candidate.end(); iter++) {
        cout << iter->first << ' '; //사진이 게재된 최종 후보의 학생 번호를 증가하는 순서대로 출력
    }
    return 0;
}