...
ci dfs(int node, int parent, vector<vector<ci>>& graph) {
    ...
    for (int i = 0; i < graph[node].size(); i++) { //인접한 노드의 크기만큼 탐색
        int next_node = graph[node][i].first;
        if (next_node == parent) { //다음 노드가 부모 노드이면
            continue; //continue
        }
        ...
        if (graph[node][i].second + dfs_search.second > len) { //기존 거리보다 길다면 갱신
            ...
            pos = dfs_search.first; //지금을 구성하는 노드 갱신
        }
    }
    return { pos, len }; //가장 먼 거리와 그 인덱스를 리턴
}
 ...
 *
 * 부모->자식의 방향만 저장하면 리프 노드에서 다른 리프 노드로 탐색할 수 없으므로 
     무방향 그래프로 저장
 */
...
int main() {
    ...
    for (int i = 1; i < n; i++) { //무방향 그래프로 만들기
        cin >> p >> c >> w; //부모 노드의 번호, 자식 노드, 간선의 가중치
