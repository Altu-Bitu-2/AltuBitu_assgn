...
void second(int n, int start, int end) {
    ...
    while (cur != start) { //end - 1 부터 start 까지 검사
        ...
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { 
            belt[cur].is_on = false; // 현재 위치에 로봇 X
            belt[next].power--; // 내구성 감소
            if (next != end) { //내리는 위치 아닐 경우만
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) { // 다음 위치에 내구도가 0이면
                zero_power++; // 내구도가 0인 벨트 칸의 개수 증가
            }
        }
    }
}
...
void third(int start) {
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
        belt[start].is_on = true; //현재 위치에 로봇 O
        belt[start].power--; //내구성 감소
 ...
 * 2번: 가장 먼저 올라간 로봇부터 고려해야 하므로 (내리는 위치 - 1)부터 (올리는 위치)까지 검사 
    -> 로봇 옮기는 거 가능하면 존재여부 체크하고 내구도 감소
...
int main() {
    ...
    while (true) {
        ...
        if (belt[end].is_on) { //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false; // 내리는 위치에 로봇 X
        }
        second(n, start, end); //(내리는 위치 - 1) 부터 (올리는 위치)까지 검사하는 함수
        third(start); //올리는 칸의 내구도가 0이 아니라면 해당 칸의 로봇 존재 여부를 체크하고 내구도를 감소시키는 함수
        step++; //단계 수 증가
        if (zero_power >= k) { //내구도가 0인 칸의 수가 k 이상이면
            break; //종료
        }
    }
