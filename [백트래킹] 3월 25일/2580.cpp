...
bool fillSudoku(int idx) {
    ...
    for (int i = 1; i <= MAX; i++) { //1~9까지 넣어보기
        //각 행의 숫자 존재 X, 각 행의 열의 숫자 존재 X, 각 3x3 사각형의 숫자 존재 X
        if (!check_row[row][i] && !check_col[col][i] && !check_3x3[(row / 3) * 3 + col / 3][i]) {
            check_row[row][i] = true; //i번째 행 숫자 존재
            check_col[col][i] = true; //i번째 열 숫자 존재
            check_3x3[(row / 3) * 3 + col / 3][i] = true; //i번째 3x3 사각형에 숫자 존재
            sudoku[row][col] = i;
            if (fillSudoku(idx + 1)) {
                return true; //첫번째와 마지막 true
            }
            check_row[row][i] = false; //i번째 행 숫자 존재 X
            check_col[col][i] = false; //i번째 열 숫자 존재 X
            check_3x3[(row / 3) * 3 + col / 3][i] = false; //i번째 3x3 사각형에 숫자 존재 X
            sudoku[row][col] = 0;
        }
    }
    return false;
