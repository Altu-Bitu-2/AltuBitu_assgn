...
using namespace std;
typedef pair<int, int> ci; //스트라이크와 볼의 개수를 담은 pair
...
int game(int n, vector<baseball>& question) {
    ...
    for (int i = 123; i <= 987; i++) { //가능한 최소 숫자: 123, 가능한 최대 숫자: 987
        bool check = true; //
...
int main() {
    ...
    //입력
    cin >> n; //민혁이나 영수에게 몇 번 질문했는지
    
    vector<baseball> question(n); //baseball struct 가진 vector 설정
    for (int i = 0; i < n; i++) {
        cin >> question[i].num >> question[i].strike >> question[i].ball; //질문한 세자리 수, 스트라이크 개수, 볼 개수
    }
